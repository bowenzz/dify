...
const ApiBasedExtensionModal: FC<ApiBasedExtensionModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (localeData && localeData.api_key && localeData.api_key?.length < 5) {
      notify({ type: 'error', message: t('common.blockchain.modal.apiKey.lengthError') })
      setLoading(false)
      return
    }
  ...
  return (
    ...
    >
      <div className='mb-2 text-xl font-semibold text-text-primary'>
        {
          data.name
            ? t('common.blockchain.modal.editTitle')
            : t('common.blockchain.modal.title')
        }
      </div>
      <div className='py-2'>
        <div className='leading-9 text-sm font-medium text-text-primary'>
          {t('common.blockchain.modal.name.title')}
        </div>
        <input
          ...
          placeholder={t('common.blockchain.modal.name.placeholder') || ''}
        />
      </div>
      <div className='py-2'>
        <div className='flex justify-between items-center h-9 text-sm font-medium text-text-primary'>
          {t('common.blockchain.modal.apiEndpoint.title')}
          <a
            href={t('common.blockchain.linkUrl') || '/'}
        ...
        <input
          ...
          className='block px-3 w-full h-9 bg-components-input-bg-normal rounded-lg text-sm text-text-primary outline-none appearance-none'
          placeholder={t('common.blockchain.modal.apiEndpoint.placeholder') || ''}
        />
      </div>
      <div className='py-2'>
        <div className='leading-9 text-sm font-medium text-text-primary'>
          {t('common.blockchain.modal.apiKey.title')}
        </div>
        <div className='flex items-center'>
          <input
            ...
            placeholder={t('common.blockchain.modal.apiKey.placeholder') || ''}
          />
        </div>
      </div>
